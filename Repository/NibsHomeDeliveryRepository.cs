...
{
    public class NibsHomeDeliveryRepository
    {
            ...
            if (Type == "Half")
            {
                if (items.Count() > 0)
                {
                    foreach (XElement itemElement in items)
                    {
                        ...
                        //decimal TotalAmount = (Convert.ToDecimal(itemElement.Element("TotalAmount").Value) + Convert.ToDecimal(result.HalfPrice))*Convert.ToDecimal(Qty);
                ...
                {
                    //var vatamount = (Convert.ToDecimal(result.Vat) / 100) * Convert.ToDecimal(result.HalfPrice) * Convert.ToDecimal(Qty);
        ...
        public GetBillingModel GetBillingItem(string Path, string KotPath)
        {
            ...
            model.ServicesCharge = Math.Round(ServiceChargeAmount, 2);
            model.NetAmount = Math.Round(model.TotalAmount + model.VatAmount + model.ServicesCharge, 2);
        ...
        public bool _TransferToReturnXML(string TablePath, string ReturnxmlPath, string KotFilePath, int ItemId = 0)
        {
            ...
            if (returnitems.Count > 0)
            {
                foreach (XElement itemElement in items)
                {
                    ...
                    itemElement.SetElementValue("TokenNo", TokenNo.ToString());
            ...
            else
            {
                foreach (XElement itemElement in items)
                {
                    var newElement = new XElement("Items",
                            ...
                            new XElement("TokenNo", TokenNo.ToString()),
        ...
        public PrintBillModel GetBill(string Path, GetBillingModel m)
        {
            ...
            foreach (var item in VatDetail)
            {
                ...
                pm.Vat = Math.Round(Convert.ToDecimal(item.Vat),2);
                VatList.Add(pm);
            }
