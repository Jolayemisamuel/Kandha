...
namespace NibsMVC.Repository
{
    public class TakeawayRepository
    {
        ...
        #region Cashier & Manager(Outlet)
        public List<TakeAwayModel> GetAllCategoryList()
        {
            ...
            List<TakeAwayModel> list = new List<TakeAwayModel>();
            foreach (var item in category)
            {
                TakeAwayModel model = new TakeAwayModel();
        ...
        public string DispatchOrder(OrderTakeDispatchModel model, string Path)
        {
            try
            {
                int oulte = getOutletId();
