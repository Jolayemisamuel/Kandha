$('input[name=CategoryName]').on('click', function () {
    var Id = $(this).attr('id');
    var FormData = {Id:Id}
    $.post('/TakeAway/GetAllItem' , FormData, function (data) {
        $('#Items').html(data);
    });
});
$(document).on('click', 'input[name="itemName"]', function () {
    ...
    $.post('/TakeAway/GetXmlData', formData, function (data) {
        BindData(data);
    });
});
...
$(document).on('click', '.deleterow', function () {
    var el = $(this).attr('id');
    var TokenNo = $("#RunningTable").val();
    var FormData = { Id: el, TokenNo: TokenNo }
    $.post('/TakeAway/DeleteNode',FormData, function (data) {
        BindData(data);
    });
});
...
$(document).on('click', '#btnPrint', function () {
    ...
    $.post('/TakeAway/ClearKot', FormData, function (data) {
        BindData(data);
        $('.modal').modal('hide');
    });
});
function BindData(data)
{
    ...
    $('#txtvatamt').val(Math.round(TotalVatAmount, 2));
    $('#txtsubtotal').val(Math.round(TOtalAmount, 2));
    var Takecharge = $("#txtservice").val();
...
$(document).on('click', '#canclegorder', function () {
    var TokenNo = $("#RunningTable").val();
    var formData = { TokenNo: TokenNo };
    //$.getJSON('/TakeAway/CencelOrder/' + Id, function (data) {
    //    $('#BindXmlData').html("");
    //    $('#txtvatamt').val("");
    //    $('#txtsubtotal').val("");
    //    $('#txtnettotal').val("");
